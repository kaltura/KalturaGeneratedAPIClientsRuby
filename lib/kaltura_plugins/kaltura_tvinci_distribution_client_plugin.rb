...
module Kaltura
	...
	class KalturaTvinciDistributionProfile < KalturaConfigurableDistributionProfile
		...
		attr_accessor :assets_type

		def assets_type=(val)
			@assets_type = val.to_i
		end
		...
		def from_xml(xml_element)
			...
			if xml_element.elements['innerType'] != nil
				self.inner_type = xml_element.elements['innerType'].text
			end
			if xml_element.elements['assetsType'] != nil
				self.assets_type = xml_element.elements['assetsType'].text
			end
		end
